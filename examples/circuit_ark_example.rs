...
impl<F: Field + PrimeField> StepCircuit<F> for ArkCircuit<F> {
  ...
  fn synthesize<CS: ConstraintSystem<F>>(
    ...
    let poseidon_c = vec![
      ...
      .unwrap(), // 0xee9a592ba9a9518d05986d656f40c2114c4993c11bb29938d21d47304cd8e6e
      F::from_str_vartime(
        "426281677759936592021316809065178817848084678679510574715894138690250139748",
      )
      .unwrap(), // 0xf1445235f2148c5986587169fc1bcd887b08d4d00868df5696fff40956e864
      F::from_str_vartime(
        "4014188762916583598888942667424965430287497824629657219807941460227372577781",
      )
      .unwrap(), // 0x8dff3487e8ac99e1f29a058d0fa80b930c728730b7ab36ce879f3890ecf73f5
    ];
...
fn main() {
  ...
  let z0 = vec![Fr::from(0u64), Fr::from(1u64), Fr::from(2u64)];

  let pp =
    PublicParams::<E1, E2, ArkCircuit<Fr>>::setup(&circuit, &*S1::ck_floor(), &*S2::ck_floor())
      .unwrap();

  let mut snark = RecursiveSNARK::new(&pp, &circuit, &z0).unwrap();
